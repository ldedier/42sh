...
t_list	*handle_brace_tok(t_list *e)
{
	t_list	*rbrace;
	t_list	*prev;
	...
	{
		prev = e;
		while (prev->next && prev->next != rbrace)
			prev = prev->next;
		prev->next = rbrace->next;
		free(rbrace);
		e = e->next;
		return (e);
	}
...
t_list	*handle_compound_tok(t_list *e, t_symbol_id start_symb)
{
	// int				count;
	t_symbol_id		id;
	// t_token			*t;
